...
if (isIndexPage) {
  ...
  // Tracks currently selected skills
  var selectedSkills = [];
  ...
  function isSkillSelected(skill) {
    ...
    return selectedSkills.some(function (s) {
      return normalizeSkill(s) === normalizedSkill;
    });
  }

  function getCanonicalSkill(rawSkill) {
    var normalizedSkill = normalizeSkill(rawSkill);
    var matched = availableSkills.find(function (skill) {
  ...
  // Keyboard navigation in text input
  ...
  // Click on a quick-pick chip → add/toggle skill
  quickPickChips.forEach(function (chip) {
  ...
  // Render selected skill capsules inside the input box
  function renderSelectedChips() {
    chipsSelectedEl.innerHTML = "";

    // Hide placeholder when at least one skill is selected
    if (selectedSkills.length > 0) {
      skillsTextInput.placeholder = "";
    } else {
      skillsTextInput.placeholder = "Type a skill and press Enter...";
    }
...
if (
  ...
  fetchBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      var langs = repos
        .map(function (r) { return r.language; })
        .filter(Boolean)
        .filter(function (lang, idx, arr) { return arr.indexOf(lang) === idx; });
